...
def run(_context: str):
 ...
 options=a.importManager.createFusionArchiveImportOptions(os.path.join(BASE,'cad','Energized_Water_Scanner_RevB3.f3d'))
 ...
 with open(os.path.join(BASE,'verification','RevB3_Native_Roundtrip.json'),'w') as f:json.dump(result,f,indent=2)
 assert result['passed'],result
 doc.close(False);original.activate()
 assert original.save('Rev-B.3: battery tray screw-head clearance, ESP32 header access, motor pilot depth; E1 and hardware qualification gates remain open')

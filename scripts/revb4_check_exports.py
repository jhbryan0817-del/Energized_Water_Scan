"""Verify Rev-B.4 export consistency; run after Fusion audits and mesh export."""
...
def main():
    audit=json.loads((ROOT/'verification/RevB4_Assembly_Audit.json').read_text())
    meshes=json.loads((ROOT/'verification/RevB4_STL_Check.json').read_text())
    native=json.loads((ROOT/'verification/RevB4_Native_Roundtrip.json').read_text())
    step=(ROOT/'cad/Energized_Water_Scanner_RevB4.step').read_text()
    ...
    files=[*ROOT.glob('cad/*RevB4*'),*ROOT.glob('prints/*.stl'),ROOT/'prints/Print_STLs.zip',*ROOT.glob('previews/RevB4*'),*ROOT.glob('coupons/*.stl')]
    ...
    (ROOT/'verification/RevB4_Export_Check.json').write_text(json.dumps(result,indent=2)+'\n')

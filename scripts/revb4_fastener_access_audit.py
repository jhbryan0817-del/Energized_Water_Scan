...
def run(_context: str):
 ...
 result={'revision':'Rev-B.4','timeline_items':d.timeline.count,'tests':tests,'unexpected_intersections':hits,'known_blocked_access':known,'assembly_release_passed':False,'E1_builder_acceptance':'Access accepted by builder on 2026-09-26; no physical seal or torque evidence supplied','boolean_failures':failures,'limits':'Head envelopes D5.5x3 M3 and D3.8x2 M2; washers and actual fastener tolerances unmodeled. Continuous header access boxes are allocations, not verified connector CAD. E1 straight-down access and the candidate low-profile wrench are blocked; builder accepts the access workaround; this is not physical sealing/torque qualification. Full release remains false due to other integration gates.'}
 with open(os.path.join(BASE,'verification','RevB4_Fastener_Access_Audit.json'),'w') as f:json.dump(result,f,indent=2)

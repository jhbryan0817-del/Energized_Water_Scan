...
def run(_context: str):
 ...
 result=dict(document=app.activeDocument.name,revision='Rev-B.3',date='2026-09-25',body_count=len(bodies),interferences_between_components=collisions,boolean_failures=failures,timeline_items=d.timeline.count,parameters=d.userParameters.count,feature_errors_and_warnings=issues,other_timeline_states=other,inventory=inventory,clearance_intersections=clearance,steering=dict(method='Rigid idealized linkage, rudder -35 to +35 degrees in 1-degree increments; flexible boot and unmodeled hardware excluded. No claim of continuous sweep or physical validation.',collisions=motion,solutions=solutions))
 with open(os.path.join(BASE,'verification','RevB3_Assembly_Audit.json'),'w') as f:json.dump(result,f,indent=2)
 ...
 assert not collisions and not failures and not issues and not motion, 'Assembly/feature/motion audit failed; inspect the saved report'
